...
function exprTerm(exprArr) {
	return (Array.isArray(exprArr) ? exprArr.filter(e => e != null || e !== '').join(';') : exprArr);
}
/**
 * Join string array
...
const visitorPatterns = {
	...
	Function(node, stack) {
		...
		if ('params' in stack) {
			let params = Array.isArray(stack.params) ? joinStatements(stack.params) : stack.params;
			return joinStatements([decl, args, params, '=', exprTerm(stack.body)]);
		} else {
			return joinStatements([decl, args,'=', exprTerm(stack.body)]);
		}
	},
	...
	EntityPlugin(node, stack) {
		...
		let params = Array.isArray(stack.params) ? stack.params : [stack.params];
		if (params) {
			return joinStatements(['plugin', stack.superclass, stack.class, ...params, '(', body, ')']);
		} else {
			return joinStatements(['plugin', stack.superclass, stack.class, '(', body, ')']);
		}
	},
	EntityPlugin_params(node, stack) {
		...
		let params = Array.isArray(stack.params) ? joinStatements(stack.params) : stack.params;
		if (params) {
			return `parameters ${stack.id} ${params}(${body})`;
		} else {
			return `parameters ${stack.id}(${body})`;
		}
	},
	PluginParam(node, stack) {
		let params = Array.isArray(stack.params) ? joinStatements(stack.params) : stack.params;
		if (params) {
			return `${stack.id} ${params}`;
		} else {
			return stack.id;
		}
	},
	...
	EntityTool(node, stack) {
		let params = Array.isArray(stack.params) ? joinStatements(stack.params) : stack.params;
		if (params) {
			return `tool ${stack.id} ${params}(${exprTerm(stack.body)})`;
		} else {
			return `tool ${stack.id}(${exprTerm(stack.body)})`;
		}
	},
	// MacroScript
	EntityMacroscript(node, stack) {
		let params = Array.isArray(stack.params) ? joinStatements(stack.params) : stack.params;
		if (params) {
			return `macroScript ${stack.id} ${params}(${exprTerm(stack.body)})`;
		} else {
			return `macroScript ${stack.id}(${exprTerm(stack.body)})`;
		}
	},
	// rcMenu
	...
	EntityRcmenu_submenu(node, stack) {
		let params = Array.isArray(stack.params) ? joinStatements(stack.params) : stack.params;
		if (params) {
			return `subMenu${stack.label}${params}(${exprTerm(stack.body)})`;
		} else {
			return `subMenu${stack.label}(${exprTerm(stack.body)})`;
		}
	},
	EntityRcmenu_menuitem(node, stack) {
		let params = Array.isArray(stack.params) ? joinStatements(stack.params) : stack.params;
		return `menuItem ${stack.id}${stack.label}${params}`;
	},
	EntityRcmenu_separator(node, stack) {
		let params = Array.isArray(stack.params) ? stack.params : [stack.params];
		if (params) {
			return joinStatements(['separator', stack.id, ...params]);
		} else {
			return joinStatements(['separator', stack.id]);
		}
	},
	// Utility - Rollout
	EntityUtility(node, stack) {
		let params = Array.isArray(stack.params) ? joinStatements(stack.params) : stack.params;
		if (params) {
			return `utility ${stack.id}${stack.title}${params}(${exprTerm(stack.body)})`;
		} else {
			return `utility ${stack.id}${stack.title}(${exprTerm(stack.body)})`;
		}
	},
	EntityRollout(node, stack) {
		let params = Array.isArray(stack.params) ? joinStatements(stack.params) : stack.params;
		if (params) {
			return `rollout ${stack.id}${stack.title}${params}(${exprTerm(stack.body)})`;
		} else {
			return `rollout ${stack.id}${stack.title}(${exprTerm(stack.body)})`;
		}
	},
	...
	EntityRolloutControl(node, stack) {
		let params = Array.isArray(stack.params) ? stack.params : [stack.params];
		if (params) {
			return joinStatements([stack.class, stack.id, stack.text, ...params]);
		} else {
			return joinStatements([stack.class, stack.id, stack.text]);
		}
	},
	// Event
	Event(node, stack) {
...
function visit(node, callbackMap) {
	...
	function _visit(node, parent, key, level = 0) {
		...
		if (nodeType !== undefined && nodeType in callbackMap) {
			...
			res = keys.map(x => stack[x]).filter(e => e != null || e !== '').join('');
		}
		//--------------------------------
		return res;
	}
